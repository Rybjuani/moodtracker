...
call "%PY_CMD%" %PY_ARGS% -m pip install pyinstaller
if errorlevel 1 (
    echo Fallo la instalacion de PyInstaller.
    exit /b 1
)
...
if exist "%ICON_FILE%" (
    ...
    call "%PY_CMD%" %PY_ARGS% -m PyInstaller --noconfirm --clean --onefile --windowed --name MoodTracker --version-file "%VERSION_FILE%" --icon "%ICON_FILE%" "%ROOT%moodtracker.py"
) else (
    echo      Icono no encontrado. Se usara el icono por defecto.
    call "%PY_CMD%" %PY_ARGS% -m PyInstaller --noconfirm --clean --onefile --windowed --name MoodTracker --version-file "%VERSION_FILE%" "%ROOT%moodtracker.py"
)
if errorlevel 1 (
