...
import json, os, sys, datetime, math, platform
...
APP_PATH  = sys.executable if IS_FROZEN else SCRIPT
        ...
        try:
            ...
            if IS_FROZEN:
                ...
                if not os.path.exists(pw):
                    pw = sys.executable
                cmd = f'"{pw}" "{SCRIPT}"'
            winreg.SetValueEx(key,"MoodTracker",0,winreg.REG_SZ,cmd)
            winreg.CloseKey(key)
...
if __name__ == "__main__":
    if IS_LINUX and not IS_FROZEN and "--detached" not in sys.argv:
        try:
