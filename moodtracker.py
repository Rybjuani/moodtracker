...
class Slider(tk.Canvas):
    ...
    def _redraw(self):
        ...
        for i in range(int(span)):
            v = -10 + 20 * i / span
            self.create_line(pad + i, ty - self.TH // 2, pad + i, ty + self.TH // 2, fill=mood_color(v))
        ...
        self.create_oval(kx - self.KR, ty - self.KR, kx + self.KR, ty + self.KR, fill=CARD, outline=col, width=2)
...
class TimelineWindow(tk.Toplevel):
    ...
    def _switch(self, tab):
        for v, b in self._btns.items():
            b.config(bg=CARD if v == tab else CARD2, fg=TEXT if v == tab else MUTED)
        for w in self._fr.winfo_children():
            w.destroy()
        self._draw(tab)
    ...
    def _mpl(self, tab, plt, mdates, FCA, LSC):
        ...
        if tab == "entries":
            ...
            if not es:
                ...
                ax.scatter(dts, vals, c=vals, cmap=cmap, vmin=-10, vmax=10, s=50, zorder=5, edgecolors=BG, linewidths=0.8)
            ...
            d = dict(self.data["daily"])
            avg, _ = today_stats(self.data)
            if avg is not None:
                d[today] = avg
            if not d:
                ...
                dates = sorted(d.keys())
                vals = [d[x] for x in dates]
                xs = [datetime.date.fromisoformat(x) for x in dates]
                bars = ax.bar(xs, vals, color=bc(vals), alpha=0.9, width=0.7, edgecolor=BG, linewidth=0.4)
            ...
            if not w:
                ...
                wks = sorted(w.keys())
                vals = [w[x] for x in wks]
                ys = list(range(len(wks)))
                ax.barh(ys, vals, color=bc(vals), alpha=0.9, edgecolor=BG)
                ax.set_yticks(ys)
                ax.set_yticklabels(wks, color=MUTED, fontsize=8)
                ax.set_xlim(-12, 12)
            ...
            if not m:
                ...
                mos = sorted(m.keys())
                vals = [m[x] for x in mos]
                ys = list(range(len(mos)))
                ax.barh(ys, vals, color=bc(vals), alpha=0.9, edgecolor=BG)
                ax.set_yticks(ys)
                ax.set_yticklabels(mos, color=MUTED, fontsize=9)
                ax.set_xlim(-12, 12)
                ax.axvline(0, color=BORDER, linewidth=1.2)
    ...
    def _tkdraw(self, tab):
        today = datetime.date.today().isoformat()
        if tab == "entries":
            items = [(datetime.datetime.fromisoformat(e["timestamp"]).strftime("%d/%m %H:%M"), e["value"]) for e in self.data["entries"]]
            ...
            items = sorted(self.data["weekly"].items())
            title = "Promedio Semanal"
        else:
            items = sorted(self.data["monthly"].items())
        ...
        n = len(items)
        bw = max(5, min(32, (pw - 8) // n - 2))
        for i, (lbl, val) in enumerate(items):
            x = PL + 8 + i * (pw - 8) // n + bw // 2
            yv = cy0 - int(val * (ph // 2) / 10)
            col = mood_color(val)
            c.create_rectangle(x - bw // 2, min(yv, cy0), x + bw // 2, max(yv, cy0), fill=col, outline="")
            c.create_text(
                x,
                yv + (-10 if val >= 0 else 10),
                text=f"{val:.1f}" if isinstance(val, float) else str(val),
                fill=TEXT,
                font=(FN, 7),
            )
            if i % max(1, n // 12) == 0:
                c.create_text(x, H - PB + 12, text=str(lbl)[:7], fill=MUTED, font=(FN, 7))
...
def setup_autostart():
    if IS_WIN:
        ...
        messagebox.showinfo("Inicio Automático", "✅ Listo!\n\nMoodTracker se iniciará automáticamente.")
...
class MoodWidget:
    ...
    def _lbl(self, parent, text, fg=None, font_size=9, bold=False, anchor="center"):
        ...
        return tk.Label(parent, text=text, bg=CARD, fg=fg or MUTED, font=f, anchor=anchor)
    ...
    def _build(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="MOOD", bg=CARD2, fg=ACCENT, font=(FN, 8, "bold")).pack(side="left")
        tk.Label(hdr, text="TRACKER", bg=CARD2, fg=MUTED, font=(FN, 8)).pack(side="left", padx=(3, 0))
        ...
        self._desc_lbl = tk.Label(mid, text="Neutral", bg=CARD, fg=MUTED, font=(FN, 8))
        self._desc_lbl.pack(pady=(2, 0))

        sf = tk.Frame(m, bg=CARD, padx=12, pady=4)
        sf.pack(fill="x")
        lr = tk.Frame(sf, bg=CARD)
        lr.pack(fill="x")
        tk.Label(lr, text="−10", bg=CARD, fg="#be1414", font=(FN, 7)).pack(side="left")
        tk.Label(lr, text="+10", bg=CARD, fg="#0f9637", font=(FN, 7)).pack(side="right")
        ...
        st.pack(fill="x")
        self._avg_lbl = tk.Label(st, text="Promedio hoy:  —", bg=CARD, fg=TEXT, font=(FN, 9, "bold"), anchor="w")
        self._avg_lbl.pack(fill="x")
        self._cnt_lbl = tk.Label(st, text="Sin registros aún", bg=CARD, fg=MUTED, font=(FN, 8), anchor="w")
        ...
        bot.pack(fill="x")
        for txt, cmd in [("📊 Historial", self._timeline), ("⚙ Inicio auto", setup_autostart)]:
            tk.Button(
                bot,
                text=txt,
                command=cmd,
    ...
    def _refresh(self):
        ...
        for w in self._hist_fr.winfo_children():
            w.destroy()
        ...
        for e in last:
            ...
            tk.Label(pill, text=f"{e['value']:+d}", bg=CARD2, fg=col, font=(FN, 8, "bold")).pack()
            tk.Label(pill, text=t, bg=CARD2, fg=MUTED, font=(FN, 7)).pack()
    ...
    if IS_LINUX and "--detached" not in sys.argv:
        try:
            if sys.stdin and os.isatty(sys.stdin.fileno()):
                ...
                cmd = [APP_PATH, "--detached"] if getattr(sys, "frozen", False) else [sys.executable, APP_PATH, "--detached"]
                subprocess.Popen(
                    cmd,
